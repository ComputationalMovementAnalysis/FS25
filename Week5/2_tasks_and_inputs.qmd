...
1. Calculate BMI based on a person's height and weight ($\text{BMI} = \frac{\text{Weight (kg)}}{\text{Height (m)}}$)
2. Calculate Degrees Farenheight based on Degrees Celcius ($\text{Farenheight} = \text{Celcius} \times \frac{9}{5} + 32$)

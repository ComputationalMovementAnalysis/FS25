...
If you already have a Github account (e.g. for work) and don't want your GitHub activities from our course associated with your existing account, we recommend you create *a new Github account with your student mail address* (`username@students.zhaw.ch`/`username@uzh.ch`). If you know that the laptop you are currently using is only for student work, you can set your `user.email` to your student mailadress. However, if you expect to work with both GitHub accounts with your current laptop, you have to set `user.email` on a *per project* basis (by omitting the `--global` flag).
